...
class TestCommonDoc(BaseTestCase):
    ...
    def test_find_common_base_dir_dblayer(self):
    ...
    def test_find_common_base_dir_langchain(self):
        path = os.path.join(self.data_dir, 'langchain_archive_paths.txt')
        with open(path, 'rt', encoding='ascii') as f:
            langchain_files = f.read().splitlines()
        common_base_dir = find_common_base_dir(langchain_files)
