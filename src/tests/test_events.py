...
class TestProducerConsumer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_produce_consume_fail(self):
        async with asyncpg.create_pool(self.dsn, command_timeout=60) as pool:
            ...
            async def error_handler(event: Dict[str, Any], message: str):
                errors.append(message)
                self.assertEqual(event['type'], 'event2')
                self.assertEqual(event['params'], '{"param1": "value1"}')
                self.assertEqual(message, 'Test Exception')

            consumer.register_event_handler('event2', event_handler)
            consumer.register_event_handler('Error', error_handler)
